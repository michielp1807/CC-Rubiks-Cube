...
local function newBuffer(x1, y1, x2, y2)
	...
	function buffer:loadLineBLittle(x1, y1, x2, y2, c, a, b)
		...
		if x2 >= x1 then
			for x = max(ceil(x1), 1), min(floor(x2), frameWidth) do
				local y = floor(a * x + b + 0.5)
				if y > 0 and y <= frameHeight then c2[y][x] = c end
			end
		else
			for x = max(ceil(x2), 1), min(floor(x1), frameWidth) do
				local y = floor(a * x + b + 0.5)
		...
		if y2 >= y1 then
			for y = max(ceil(y1), 1), min(floor(y2), frameHeight) do
				local x = floor((y - b) / a + 0.5)
				if x > 0 and x <= frameWidth then c2[y][x] = c end
			end
		else
			for y = max(ceil(y2), 1), min(floor(y1), frameHeight) do
				local x = floor((y - b) / a + 0.5)
	...
	function buffer:drawTriangleBLittle(x1, y1, x2, y2, x3, y3, c, char, charc, outlineColor)
		...
		if y1 <= y2 and y1 <= y3 then
			if y2 <= y3 then
				if a1 ~= 0 then horLine(self, a1, b1, a3, b3, y1, y2, c) end
				if a2 ~= 0 then horLine(self, a2, b2, a3, b3, y2, y3, c) end
			else
				if a3 ~= 0 then horLine(self, a1, b1, a3, b3, y1, y3, c) end
				if a2 ~= 0 then horLine(self, a1, b1, a2, b2, y3, y2, c) end
			end
		elseif y2 <= y1 and y2 <= y3 then
			if y1 <= y3 then
				if a1 ~= 0 then horLine(self, a1, b1, a2, b2, y2, y1, c) end
				if a3 ~= 0 then horLine(self, a2, b2, a3, b3, y1, y3, c) end
			else
				if a2 ~= 0 then horLine(self, a1, b1, a2, b2, y2, y3, c) end
				if a3 ~= 0 then horLine(self, a1, b1, a3, b3, y3, y1, c) end
			end
		else
			if y1 <= y2 then
				if a3 ~= 0 then horLine(self, a2, b2, a3, b3, y3, y1, c) end
				if a1 ~= 0 then horLine(self, a1, b1, a2, b2, y1, y2, c) end
			else
				if a2 ~= 0 then horLine(self, a2, b2, a3, b3, y3, y2, c) end
				if a1 ~= 0 then horLine(self, a1, b1, a3, b3, y2, y1, c) end
			end
		end
	...
	function buffer:drawBufferBLittle()
		...
		if not blittleWindow then
			self.blittleWindow = window.create(term.current(),
				self.x1, self.y1, self.x1 + self.width - 1, self.y1 + self.height - 1, false)
						...
						if onScreen3 then
							if polygon[10] or (x2 - x1) * (y3 - y2) - (y2 - y1) * (x3 - x2) < 0 then
								if not self.blittleOn then
									...
									if isInTriangle(x, y, x1, y1, x2, y2, x3, y3, (self.x2 - 1) * 2 + 1, (self.y1 - 1) * 3 + 1, (self.x2) * 2,
										(self.y2 + 1) * 3) then
										solutions[#solutions + 1] = { objectIndex = i, polygonIndex = polygon[15] }
									end
								end
	...
	for i = 1, #model do
		...
		if not options.colorsFractal then
			...
			if (options.colors) then
				...
				if (avgY >= 0) then poly.c = options.top or poly.c
				else poly.c = options.bottom or poly.c end
			end
		else
...
function models:plane(options)
	...
	return {
		...
		newPoly(-1 * options.size, options.y, 1 * options.size, 1 * options.size, options.y, 1 * options.size, 1 * options.size
			, options.y, -1 * options.size, options.color),
	}
end
